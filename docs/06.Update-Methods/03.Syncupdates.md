...
* **HELP** <br> 
This variable can be either yes or no. If it contains yes, the help message of the Whois server will be given as the output. A HELP request can be sent using the GET or POST method.

Multiple objects are accepted in the same request, but each object must be separated by an empty line.

After composing the input and sending it as a **POST** request, the server will return the usual HTTP response headers and the body if an acknowledgement exists.
...
Examples of GET help request:

    curl -v -d "HELP=yes" https://syncupdates-test.db.ripe.net
    
    curl -v https://syncupdates-test.db.ripe.net/?HELP=yes

Example of a POST update request:
...
To test an update without affecting the underlying database, you can use [dry-run](../Updating-Objects-in-the-RIPE-Database/Dry-run/#dry-run) in the request body.
