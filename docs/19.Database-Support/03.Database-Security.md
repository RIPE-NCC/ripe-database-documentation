...
Lost access to your maintainer? This [web interface](https://apps.db.ripe.net/db-web-ui/fmp) allows you to recover access to a maintainer. Please note that you will be required to log in with your RIPE NCC Access (SSO) account. You can create one if you do not have one yet.
...
**organisation** object. If you had a previous default maintainer the new default maintainer will replace your previous one (i.e. there can only be one default maintainer at a time).
...
Keep in mind that sharing a maintainer doesn't mean sharing the credentials as well, such as an SSO account or a PGP key or X.509 certificate. As explained earlier, a maintainer can contain multiple authentication attributes, so each person that has access to it should use their own set of credentials.
