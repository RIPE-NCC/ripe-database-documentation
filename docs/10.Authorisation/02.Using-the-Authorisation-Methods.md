...
A signed update message can also be pasted into the Syncupdates web form. When using HTTP POST requests for Syncupdates, the signed update must be supplied in the message body.
...
Remember that if you have multiple "auth:" attributes in a **maintainer** or if you have multiple "mnt-by:" attributes in an object, all possible authentication methods are combined by a logical OR which means that any single one of the specified authentication methods can be used.
