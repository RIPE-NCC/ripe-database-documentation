...
A maintainer of an object in the RIPE Database is an entity, which can alone make changes to that object. A **mntner** object represents the maintainer. The maintainer authenticates change request either by using a PGP-Key or SSO account as specified in "auth:" attribute.
...
You can create one by sending a completed template using [Mailupdates](../Update-Methods/Email-Updates) or [Syncupdates](../Update-Methods/Syncupdates).
...
You start the process of creating an **irt** object by sending a completed template by e-mail to the RIPE NCC. As creation of an **irt** object is a manual process, you must address your email to the Database support team [ripe-dbm@ripe.net](mailto:ripe-dbm@ripe.net)
...
Authorisation can be provided by a PGP signature or SSO account. All possibilities are represented by the "auth:" field in the **irt** object.

You can submit the update using [Mailupdates](../Update-Methods/Email-Updates) or [Syncupdates](../Update-Methods/Syncupdates).

### Link a Single INET(6)NUM Object to an IRT Object

You can link a single **inet(6)num** object to an IRT object by adding a "mnt-irt:" attribute.
...
Both need to sign the request. It does not matter in which order this signing occurs as long as both parties sign the request.

#### If Only One Party Uses SSO for Authorisation

In this case, whoever is using PGP should initially sign the request and send it to the other. The other party then logs in to RIPE NCC Access to also authorise the request and then send the request using [Syncupdates](../Update-Methods/Syncupdates).

#### If Both Parties Use SSO Authorisation
...
In this case, the process is straightforward, as the entire change is authenticated by the same maintainer.
...
Exact the **inet(6)num** object from the RIPE Database:
    ...
    whois -h whois.ripe.net -Br <ipadress> > inetnum.txt

Alternatively you can search for any other entry in that object with -i, for example for the "mntner:" attribute.

    whois -h whois.ripe.net -i mnt-by <mntner-name> -Br > inetnum.txt


**Here is an example:**

    $ whois -h whois.ripe.net -Br 193.174.13.99 > inetnum.txt
...
Let the other maintainer also authenticate the request by either signing the file (inetnum.txt.asc) with the PGP key referenced in his **mntner** object. An alternative would be to authenticate it by login to RIPE NCC Access and submit using [Syncupdates](../Update-Methods/Syncupdates).

Send this authenticated request using [Mailupdates](../Update-Methods/Email-Updates) or [Syncupdates](../Update-Methods/Syncupdates).
...
You should collect all the **inet(6)num** objects into one list. Make the appropriate changes and send the change request using [Mailupdates](../Update-Methods/Email-Updates) or [Syncupdates](../Update-Methods/Syncupdates).

### Some INET(6)NUM Objects Have Different Maintainer Objects

You need authorisation from each party involved. You should collect the **inet(6)num** objects maintained by different maintainers into separate lists. Authorise these separately and then send each using [Mailupdates](../Update-Methods/Email-Updates) or [Syncupdates](../Update-Methods/Syncupdates).
...
Extract all the **inet(6)num** objects belonging to the **mntner** object from the RIPE Database:
    ...
    whois -h whois.ripe.net -Br -i mnt-by <maintainer name> > inetnums.txt

You will see a list of objects, separated by an empty line.

* **Here is an example:**

    $ whois -h whois.ripe.net -Br -i mnt-by DFN-LIR-MNT > inetnums.txt
...
Send that file to the other maintainer. They should authenticate the request by either signing the file (inetnum.txt.asc) with the PGP key referenced in his **mntner** object or send the file using [Mailupdates](../Update-Methods/Email-Updates) or [Syncupdates](../Update-Methods/Syncupdates).
