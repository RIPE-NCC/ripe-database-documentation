...
module.exports = {
  ...
  referencesInsidePageFormatter: function (current){
    ...
    //Link always has the next structure: [*](*#*) or [*](*). We have to remove the * before # because now we are
    // pointing to the same doc
    var regex = /(\[.+?(?=\]\())(.+?(?=(\#(.+?(?=\))))|\)))/g;
    return current.content.replaceAll(regex, function(matchingWord,firstMatchingPart,secondMatchingPart,thirdMatchingPart){
