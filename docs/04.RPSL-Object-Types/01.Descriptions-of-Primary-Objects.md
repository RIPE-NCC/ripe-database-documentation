...
* **"prefixlen:"** A URL referencing a CSV file (described by [draft-ietf-opsawg-prefix-lengths](https://datatracker.ietf.org/doc/draft-ietf-opsawg-prefix-lengths/)) containing additional metadata about how the prefix is used or subdivided. The URL must be well formed and it must specify the HTTPS protocol.
...
* **“status:”** – The status indicates the type of data stored in an **inet6num** object and the relative positions within a hierarchy. It can not be changed after creating the object.
  This documentation is intended for advisory purposes only. For authoritative definitions and rules, please refer to the official [IPv6 Address Allocation and Assignment Policy](https://www.ripe.net/publications/docs/ripe-738/).
...
With the **inet6num** object there is no equivalent to the **inetnum** ‘LIR-PARTITIONED' status values allowing partitioning of an allocation by a member for internal business reasons.
    * AGGREGATED-BY-LIR – With IPv6, This status indicates that it is not necessary to document each End User assignment in the RIPE Database.
      If you have a group of End Users who all require blocks of the same size and are uniform - where the purpose and contact information are the same -, say a /56, then you can create a large, single block with this status.

      The "assignment-size:" attribute is required for this status and specifies the size of the End User blocks. All assignments made from this block must have that size. It is possible to have two levels of ‘AGGREGATED-BY-LIR'. The "assignment-size" value cannot be changed after the **inet6num** object has been created. ![](/imgs/assignment-size.png)

      Individual End User assignments do not need to be represented in the RIPE Database. However, a non uniform assignment or an assignment of a different size may still be created beneath this AGGREGATED-BY-LIR object.
    * ASSIGNED – These are assignments made by a member from their allocations to an End User. ![](/imgs/assigned.png)
...
* **“assignment-size:”** – This specifies the common size of all individual assignments aggregated into one block with the status ‘AGGREGATED-BY-LIR'. This attribute is required to be present if the **inet6num** object has this status. The maximum assignment size for inet6num is 128.
...
* **"abuse-c:"** - This attribute references an abuse contact object. If present, this overrides any existing referenced **organisation**, or * **"abuse-c:"** attribute present in the hierarchy of this object . This can only be a **role** object that contains an "abuse-mailbox:" attribute. Making this reference will remove any query limits for that **role** object, which must only include business data (no personal information).
* **"status:"** - The status attribute indicates the type of data stored in an **inetnum** object and its relative position within the address hierarchy.
  This documentation is intended for advisory purposes only. For authoritative definitions and rules, please refer to the official [IPv4 Address Allocation and Assignment Policy](https://www.ripe.net/publications/docs/ripe-826/#70-types-of-address-space).
  ...
  The status attribute can take one of the following values:
    ...
    * 'AGGREGATED-BY-LIR' - This status indicates that it is not necessary to document each individual End User assignment in the RIPE Database. If you have a group of End Users with uniform assignments - where the purpose and contact information are the same - you may create a single aggregated block covering the appropriate address range and assign it to the AGGREGATE-BY-LIR status. 
      
      If, in addition, all End Users assignments are of the same size, you may also include the "assignment-size:" attribute to specify the size of the individual End User blocks.
      
      Individual End User assignments do not need to be represented in the RIPE Database. However, a non uniform assignment may still be created beneath this AGGREGATED-BY-LIR object.
...
* **“assignment-size:”** – This attribute specifies the common size of all individual assignments aggregated into one block with the status ‘AGGREGATED-BY-LIR'. This attribute is optional when the **inetnum** object has this status. The "assignment-size" value cannot be changed after the **inetnum** object has been created. The maximum assignment size for inetnum is 32. 
...
Below is the object template for the **filter-set** object. It lists all possible attributes that are allowed in this object type. Required attributes are shown as 'mandatory'.
    ...
    descr:          optional    multiple
    filter:         conditional single  
    mp-filter:      conditional single     
    remarks:        optional    multiple  
    org:            optional    multiple   inverse key
    tech-c:         mandatory   multiple   inverse key
    admin-c:        mandatory   multiple   inverse key
