...
* **"contact:** - This is a contact method represented by a URI (for example, but not limited to: WhatsApp, Signal, Telegram, SIP).
...
Objects in the RIPE Database are protected using **mntner** objects. A **mntner** object is an anonymous box containing the credentials needed to authorise creation, deletion or modification of any objects that it protects by whomever maintains this data. Currently, these credentials are PGP keys or X.509 certificates or Single Sign-On accounts from the RIPE NCC Access system.
...
Below is the object template for the **organisation** object. It lists all possible attributes that are allowed in this object type.
    ...
    phone:            conditional multiple  
    fax-no:           conditional multiple  
    e-mail:           mandatory   multiple   lookup key
    contact:          optional    multiple
    geoloc:           optional    single    
    language:         optional    multiple
    reg-nr:           conditional single  
    org:              optional    multiple   inverse key
    admin-c:          optional    multiple   inverse key
    tech-c:           optional    multiple   inverse key
    abuse-c:          conditional single     inverse key
...
* **"reg-nr:"** - Organisation registration number.
