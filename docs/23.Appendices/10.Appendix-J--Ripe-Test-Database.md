...
|---------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Web interface | To get started, create a role and maintainer pair. Go to  <br/> https://apps-test.db.ripe.net/db-web-ui/webupdates/create/TEST/role/self <br/> For creating a single object using a form, go to  <br/> https://apps-test.db.ripe.net/db-web-ui/#/webupdates/select |
| Mailupdates   | Email you object to test-dbm@ripe.net. <br/>You will receive an acknowledgement message with the result.                                                                                                                                                           |
| Syncupdates   | For creating one or more objects at a time, using a single field to edit objects. Go to https://apps-test.db.ripe.net/db-web-ui/syncupdates  <br/>You can also submit updates from the command line (or script) via: https://syncupdates-test.db.ripe.net/         |
| REST API      | Software API to create a single object at a time.  Documented in [Whois REST API](../Update-Methods/RESTful-API/#ripe-database-restful-api). <br/>The base URL for the TEST environment is https://rest-test.db.ripe.net/test                                      |


## Authenticating Updates in the TEST Database

To maintain your own objects, first create a role and maintainer pair. Updates will be authenticated using your own RIPE NCC Access account.

The TEST database also allows you to authenticate as any other maintainer, which can be useful for testing. Using the web interface, when updating an object, you will be prompted to choose a maintainer if you are not already authenticated, and your RIPE NCC Access account will be added to that maintainer automatically.

You can also authenticate as an NCC maintainer using the well-known key-cert objects [PGPKEY-2B4DEF0D](https://apps-test.db.ripe.net/db-web-ui/lookup?source=test&key=PGPKEY-2B4DEF0D&type=key-cert) or [X509-1](https://apps-test.db.ripe.net/db-web-ui/lookup?source=test&key=X509-1&type=key-cert). The private key is listed in the remarks: attributes of these key-cert object. This can be useful to bootstrap the creation of your own objects, without needing to use your RIPE NCC Access account. You can use PGPKEY-2B4DEF0D with [PGP authentication](Appendix-H--PGP-Authentication-Method) to sign an update via the Mailupdates or Syncupdates interfaces. You can use X509-1 to sign an update via Mailupdates or REST API using [Client Certificate authentication](Appendix-I--Client-Certificate-Authentication).
...
You can create your own **mntner** objects in the TEST database, or use an existing maintainer. These **mntner** objects only maintain objects in the TEST database.
...
There are some RIPE NCC maintainers in the TEST database which can also be used for authentication.

|                     |                                      |
|---------------------|--------------------------------------|
| TEST-DBM-MNT        | Maintainer for TEST DBM objects.     |
| TEST-NCC-HM-MNT     | Hostmaster maintainer.               |
| TEST-NCC-END-MNT    | Maintainer for all direct resources. |
| TEST-NCC-LEGACY-MNT | Maintainer for all LEGACY resources. |
| TEST-ROOT-MNT       | Maintainer for TEST DBM maintainer.  |
...
PI'. The mntner credential for the organisation "mnt-ref:" attribute must be known in order to make a reference to the organisation. 
The organisation object must have an "org-type:" attribute with the value 'LIR' for the 'ALLOCATED PA' status 
(for example, use ORG-EIPB1-TEST as an LIR and authenticate as TEST-DBM-MNT).
...
You need to authenticate with the TEST-DBM-MNT maintainer, which maintains the root as-block object.
